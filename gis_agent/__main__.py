...
from agent_executor import GisAgentExecutor
...
SERVER_CONFIGS = {
    ...
    'usgs': {
        'command': 'python',
        'args': ['../mcp_servers/usgs-water-mcp/main.py'],
        'transport': 'stdio',
    },
    'tavily': {
        'command': 'python',
...
def main(
    ...
    """Command Line Interface to start the Gis Agent server."""
    ...
    async def run_server_async():
        async with app_lifespan(app_context):
            ...
            weather_agent_executor = GisAgentExecutor(mcp_tools = app_context.get('mcp_tools', []))
...
def get_agent_card(host: str, port: int):
    """Returns the Agent Card for the Gis Agent."""
    capabilities = AgentCapabilities(streaming=True, pushNotifications=True)
    agent_skills = [
        AgentSkill(
            id = 'flood_monitoring_information_search',
            name = 'Search flood monitoring information',
            description = """Searhces and gives current flooding conditions and reference points in the US using MCP tools""",
            tags = ['water measurements', "flood monitoring US", 'search'],
            examples = [
                'Are there any current flood warnings within Pennsylvania?',
            ],
        ),
        AgentSkill(
            id = 'measure_water_real_time',
            name = 'Search water measurements real time',
            description = """Searches and gives real-time water measurements (stream flow, gage height, temperature, etc.) using MCP tools""",
            tags = ['water measurements', 'search'],
            examples = [
    ...
    return AgentCard(
        name = 'USGS Flood monitoring Agent',
        description = 'Helps with searching water and flood monitoring information within the US',
        url = f'http://{host}:{port}/',
        version = '1.0.0',
        defaultInputModes = GisAgent.SUPPORTED_CONTENT_TYPES,
        defaultOutputModes = GisAgent.SUPPORTED_CONTENT_TYPES,
